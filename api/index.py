...
# Add root directory and current directory to sys.path so Vercel can import main
...
try:
    ...
    # Fail-safe fallback: initialize self-contained FastAPI app if root main.py is unbundled by Vercel
    ...
    def tick():
        ...
        if state["autoscaling"]:
            if state["cpu"] >= state["scale_up_threshold"] and state["instances"] < state["max_instances"]:
                ...
                state["instances"] -= 1
                add_event("Instance removed — workload returned to normal", "scale")
    ...
    @app.get("/api/")
    def api_root():
        return {"status": "healthy", "service": "cloudscale-api", "version": "1.0.0"}
